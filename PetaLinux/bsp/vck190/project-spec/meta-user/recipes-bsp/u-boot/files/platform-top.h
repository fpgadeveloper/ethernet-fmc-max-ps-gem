...
/*
 * NOTE: the PL PCS/PMA cores power up ISOLATED (control reg 0, bit 10) and
 * must be un-isolated over MDIO before they pass traffic, but that is NOT
 * done here. The GEM's MDIO controller is not usable from the u-boot
 * environment at bootcmd time: the boot-time PHY probe fails while the PHYs
 * are still in reset and tears the "eth0" MDIO bus down, so any mdio command
 * silently no-ops. The bare-metal application (xemacpsif_physpeed.c) and the
 * Linux "pcs-unisolate" boot service clear the ISOLATE bit instead. See
