...
PetaLinux. It can be built for the Ethernet FMC Max PS GEM reference designs with the
cross-platform `build.py` runner at the root of the repository, and produces a Linux image that
exercises the PS GEM Ethernet ports in exactly the same way as the PetaLinux flow.
...
1. From a command terminal, clone the Git repository (with its submodules) and `cd` into it:
   ```
   git clone --recurse-submodules https://github.com/fpgadeveloper/ethernet-fmc-max-ps-gem.git
...
| `uImage` / `Image` | Linux kernel (`Image` on Zynq UltraScale+ and Versal) |
...
## Using the PS GEM Ethernet ports

Once Linux has booted and you have logged in at the console, the PS GEM Ethernet ports are
exercised exactly as in the PetaLinux flow — see [Example Usage](petalinux.md#example-usage) for
the port-enable, fixed-IP, DHCP, status and ping walkthrough.

```{note}
**Interface names may differ from the PetaLinux flow.** The EDF rootfs uses the systemd
predictable-naming scheme, so the interface name for a given port can differ from the PetaLinux
`eth<N>` names. The interface number does **not** track the FMC port number; identify a port by
its MAC address (Ethernet FMC Max Port 0 = `00:0a:35:00:01:22`, Port 1 = `…:23`, Port 2 = `…:24`,
Port 3 = `…:25`) or by the GEM controller base address printed at boot
(`macb ff0c0000.ethernet …`). Substitute the appropriate interface name into the commands in
[Example Usage](petalinux.md#example-usage). As in the PetaLinux flow, all four PHYs share the one
MDIO bus mastered by GEM0, so the PHY appears on GEM0's MDIO controller even for a port driven by
GEM1–3.
```
...
kernel `bsp.cfg` fragments. See [advanced](advanced.md) for the full design details. The notable
ones:

* **PS GEM PHY wiring (`port-config.dtsi`).** The external Ethernet-FMC-Max PHYs are not described
  by the XSA, so each target applies a port-config overlay (`ports-0123` for the four-port ZynqMP
  designs, `ports-01--` for the two-port Versal `vck190_fmcp1`) that adds the MAC address, PHY
  handle, MDIO bus and GMII mode for each active `&gem` node. All four DP83867 PHYs share one MDIO
  bus mastered by GEM0.
* **PCS/PMA ISOLATE bit.** The PL PCS/PMA cores are not described in the device tree (the `macb`
  driver has no external-PCS support); they power up isolated, which cuts the GMII datapath until
  the bit is cleared. The `pcs-unisolate` boot service clears it — see
  [advanced](advanced.md) ("PCS/PMA ISOLATE bit and the `pcs-unisolate` service").
* **UART console mapping.** On `zcu102`, `zcu106` and `zcu111` the 2025.2 flow can map `ttyPS0`
  to the wrong PS UART, so `system-user.dtsi` pins the `uart0`/`uart1` port numbers and serial
  aliases to keep the console (cabled to UART0) deterministic.
* **PHY drivers.** Each board's `bsp.cfg` enables `CONFIG_DP83867_PHY` (the TI DP83867 PHY driver)
  and `CONFIG_XILINX_PHY` for the Ethernet FMC Max ports.
