...
files, required for the UltraZed-EV Carrier (`uzev`) target).
...
2. Clone the Git repository and `cd` into the `Vivado` folder of the repo:
   ```
   git clone --recursive https://github.com/fpgadeveloper/ethernet-fmc-max-ps-gem.git
   cd ethernet-fmc-max-ps-gem/Vivado
   ```
...
The following steps are required if you wish to build and run the [standalone application](echo_server). You can
...
1. Download the sstate-cache artefacts from the Xilinx downloads site (the same page where you downloaded
   PetaLinux tools). There are two of them:
